<div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
    <div x-data="{ mode: 'existing' }" class="space-y-4">
        <!-- Mode Selection -->
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="existing_ingredient_id" 
                            ...
                            @foreach(\App\Models\Ingredient::orderBy('name')->get() as $existingIngredient)
                                <option value="{{ $existingIngredient->id }}" data-default-unit="{{ $existingIngredient->default_unit_id }}">
                        ...
                        <div>
                            ...
                            <select name="unit_id" 
                                ...
                                @foreach(\App\Models\Unit::orderBy('name')->get() as $unit)
                                    <option value="{{ $unit->id }}">{{ $unit->name }} ({{ $unit->slug }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        <div x-show="mode === 'new'" x-transition>
            <form hx-post="{{ route('recipes.ingredients.store', $recipe) }}" 
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="sm:col-span-2">
                        <label for="name" class="block text-sm font-medium text-gray-700">New Ingredient Name</label>
                        <input type="text" 
                               name="name" 
                               id="name" 
                               required
    ...
    <script>
        ...
        document.addEventListener('change', function(e) {
            if (e.target.id === 'existing_ingredient_id') {
                const selectedOption = e.target.selectedOptions[0];
                const defaultUnitId = selectedOption?.dataset.defaultUnit;
                const unitSelect = document.getElementById('existing_unit_id');
