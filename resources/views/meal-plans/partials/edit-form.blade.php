...
<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" x-data="{ open: true }" x-show="open">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        ...
        <div class="flex items-center justify-between pb-3">
            <h3 class="text-lg font-medium text-gray-900">Edit Meal Plan</h3>
            <button type="button" 
        ...
        <!-- Modal Body -->
        <form hx-put="{{ route('meal-plans.update', $mealPlan) }}" 
              hx-target="#meal-plan-modal" 
              hx-swap="innerHTML"
              hx-on::after-request="if(event.detail.successful) { open = false; setTimeout(() => document.getElementById('meal-plan-modal').innerHTML = '', 300); location.reload(); }">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="date" 
                           ...
                           id="week_start" 
                           required
                           value="{{ $mealPlan->week_start->format('Y-m-d') }}"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
